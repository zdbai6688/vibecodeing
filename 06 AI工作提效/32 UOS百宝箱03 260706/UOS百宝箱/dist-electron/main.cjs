...
ipcMain.handle('ntp-get-status', async () => {
  ...
  try {
    ...
    return { success: true, ntpEnabled, synced, raw: lines }
  } catch(e) { return { success: false, error: e.message } }
})
...
ipcMain.handle('proxy-get-config', async () => {
  ...
  try {
    const config = {}
    try { config.http_proxy = execSync("echo $http_proxy 2>/dev/null", { timeout: 3000, encoding: 'utf-8' }).toString().trim() || execSync("gsettings get org.gnome.system.proxy.http host 2>/dev/null", { timeout: 3000, encoding: 'utf-8' }).toString().trim().replace(/'/g,'') } catch {}
    try { config.https_proxy = execSync("echo $https_proxy 2>/dev/null", { timeout: 3000, encoding: 'utf-8' }).toString().trim() } catch {}
    try { config.ftp_proxy = execSync("echo $ftp_proxy 2>/dev/null", { timeout: 3000, encoding: 'utf-8' }).toString().trim() } catch {}
    try { config.port = execSync("gsettings get org.gnome.system.proxy.http port 2>/dev/null", { timeout: 3000, encoding: 'utf-8' }).toString().trim() } catch {}
    try { config.mode = execSync("gsettings get org.gnome.system.proxy mode 2>/dev/null", { timeout: 3000, encoding: 'utf-8' }).toString().trim().replace(/'/g,'') || 'none' } catch { config.mode = 'none' }
...
ipcMain.handle('proxy-set-config', async (_, config) => {
  ...
  try {
    ...
    if (config.no_proxy) env += `no_proxy=${config.no_proxy}\nNO_PROXY=${config.no_proxy}\n`
...
ipcMain.handle('proxy-test', async (_, proxyUrl) => {
  ...
  try {
    ...
    const result = execSync(`curl -s -o /dev/null -w '%{http_code}' --proxy ${proxyUrl} --connect-timeout 10 ${testUrl} 2>/dev/null`, { timeout: 15000, encoding: 'utf-8' }).toString().trim()
    return { success: true, httpCode: result, reachable: result === '200' || result === '301' || result === '302' }
  } catch(e) { return { success: false, error: '代理连接失败' } }
})
ipcMain.handle('proxy-set-system', async (_, config, password) => {
  ...
  try {
    ...
    const host = config.http_proxy || ''
    const port = config.port || '8080'
    sudoExec(`gsettings set org.gnome.system.proxy mode manual 2>&1 && gsettings set org.gnome.system.proxy.http host '${host}' && gsettings set org.gnome.system.proxy.http port ${port}`, password)
    return { success: true }
  } catch(e) { return { success: false, error: e.message } }
})
